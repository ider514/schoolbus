...
def dash_connect():
    ...
    found_bus = db_location.find_one({'licensenumber': data['bus']})
    found_bus['route'] = data['route']
    found_school = db_schools.find_one({'username': data['school']})
    for x in range(len(found_school['buses'])):
        if found_school['buses'][x]['bus'] == found_bus['_id']:
            found_school['buses'][x]['route'] = data['route']
    ...
    if request.method == 'POST':
        ...
        for x in range(len(data['route'])):
            new_route.append({
                ...
                'lat': float(data['route'][x]['lat']),
                'lon': float(data['route'][x]['lon']),
                'here': 'false',
                'eta': 0,
                'usual_eta': 0
            })
...
def buses(school):
    ...
    bus_numbers = []
    for x in range(len(found_school['buses'])):
        bus_numbers.append(found_school['buses'][x]['bus'])
    return_buses = []
